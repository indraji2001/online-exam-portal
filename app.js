...
// ==========================================
// HTML CHECKPOINT IMPORT — PHASE 2
// ==========================================

function importHtmlCheckpoint(files) {
    // Helpers to update the status bar
    const setStatus = (icon, text, colorClass) => {
        const bar  = document.getElementById('checkpointStatus');
        const ico  = document.getElementById('checkpointStatusIcon');
        const txt  = document.getElementById('checkpointStatusText');
        if (!bar || !ico || !txt) return;
        ico.textContent = icon;
        txt.textContent = text;
        // Reset colour classes then apply new one
    ...
    // Guard: must be instructor/admin
    if (!requireRole(['admin', 'faculty'])) return;
    ...
    // Guard: must be an .html file
    if (!file.name.toLowerCase().endsWith('.html')) {
        setStatus('❌', 'Invalid file. Please choose the .html checkpoint file generated by this portal.', 'bg-red-50 border border-red-200 text-red-700');
        // Reset the file input so the user can try again
        document.getElementById('checkpointFileInput').value = '';
        return;
    }

    setStatus('⏳', `Reading "${file.name}"…`, 'bg-slate-50 border border-slate-200 text-slate-700');

    const reader = new FileReader();

    reader.onerror = () => {
        setStatus('❌', 'Could not read the file. Please try again.', 'bg-red-50 border border-red-200 text-red-700');
        document.getElementById('checkpointFileInput').value = '';
    };
    ...
    reader.onload = (e) => {
        try {
            ...
            // ── 1. Extract the hidden JSON payload ───────────────────────────
            // The payload lives inside: <script id="exam-checkpoint-data" type="application/json">…</script>
            const match = html.match(/<script[^>]+id=["']exam-checkpoint-data["'][^>]*>([\s\S]*?)<\/script>/i);

            if (!match || !match[1]) {
                setStatus('❌', 'No checkpoint data found in this file. Make sure you are uploading a file generated by the "Lock & Continue" button.', 'bg-red-50 border border-red-200 text-red-700');
                document.getElementById('checkpointFileInput').value = '';
                return;
            }
            ...
            try {
                ...
                setStatus('❌', 'Checkpoint data is corrupted or invalid JSON. Cannot restore session.', 'bg-red-50 border border-red-200 text-red-700');
                document.getElementById('checkpointFileInput').value = '';
                return;
            }

            const { cfg, sets } = payload;

            if (!cfg || !sets || typeof sets !== 'object') {
                setStatus('❌', 'Checkpoint payload is missing required fields (cfg / sets).', 'bg-red-50 border border-red-200 text-red-700');
                document.getElementById('checkpointFileInput').value = '';
                return;
            }

            // ── 3. Restore generatedSets ─────────────────────────────────────
            generatedSets = sets;

            // ── 4. Auto-fill Generate tab fields ────────────────────────────
            const setField = (id, value) => {
                const el = document.getElementById(id);
                if (el && value !== undefined && value !== null) el.value = value;
            };

            setField('genInstructor', cfg.instructor);
            setField('genCourse',     cfg.course);
            setField('genSemester',   cfg.semester);
            setField('genTopic',      cfg.topic);
            setField('genStandard',   cfg.standard);
            setField('genSets',       cfg.sets);
            setField('genAttempts',   cfg.attempts);
            setField('genDifficulty', cfg.difficulty);
            setField('genDuration',   cfg.duration);

            // ── 5. Refresh the filename preview on the Generate tab ──────────
            if (typeof updateFileNamePreview === 'function') updateFileNamePreview();

            // ── 6. Pre-fill archiveFileName on the Publish tab ───────────────
            const dateStr     = new Date().toLocaleDateString('en-GB').replace(/\//g, '');
            ...
            // ── 7. Render the question preview on the Generate tab ───────────
            if (typeof renderQuestions === 'function') renderQuestions();

            // ── 8. Show success status then navigate to Publish tab ──────────
            const totalQ = Object.values(sets).reduce((acc, arr) => acc + arr.length, 0);
            const setCount = Object.keys(sets).length;
            setStatus('✅', `Session restored! Loaded ${totalQ} questions across ${setCount} set(s). Redirecting to Publish…`, 'bg-emerald-50 border border-emerald-200 text-emerald-700');

            // Short delay so user can see the success message before navigating
            setTimeout(() => showTab('publish'), 1200);

        } catch (err) {
            console.error('importHtmlCheckpoint error:', err);
            setStatus('❌', `Unexpected error: ${err.message}`, 'bg-red-50 border border-red-200 text-red-700');
            document.getElementById('checkpointFileInput').value = '';
        }
    };
